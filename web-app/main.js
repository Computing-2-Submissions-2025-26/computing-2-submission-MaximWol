...
const codeSetterScreen = document.getElementById("code-setter-screen");
const transitionScreen = document.getElementById("transition-screen");
const codeBreakerScreen = document.getElementById("code-breaker-screen");
...
const addSecretColour = (colour) => {
    ...
    renderPegs(secretPreview, currentSecretSelection);
};
...
const addGuessColour = (colour) => {
    ...
    renderPegs(guessPreview, currentGuessSelection);
};
...
document.querySelectorAll(
    ...
    button.addEventListener("click", () => {
        addSecretColour(button.dataset.colour);
    });
});
...
document.querySelectorAll(
    ...
    button.addEventListener("click", () => {
        addGuessColour(button.dataset.colour);
    });
});

document.getElementById("secret-clear-button").addEventListener("click", () => {
    currentSecretSelection = [];
    renderPegs(secretPreview, currentSecretSelection);
});

document.getElementById("guess-clear-button").addEventListener("click", () => {
    currentGuessSelection = [];
    renderPegs(guessPreview, currentGuessSelection);
});

document.getElementById("secret-confirm-button").addEventListener("click", () => {
    if (currentSecretSelection.length !== 4) {
        gameMessage.textContent = "Secret code must contain 4 colours.";
        return;
    }
    try {
        game = Mastermind.setSecretCode(game, currentSecretSelection);
        codeSetterScreen.classList.add("hidden");
...
document.getElementById("submit-guess-button").addEventListener("click", () => {
    if (currentGuessSelection.length !== 4) {
        gameMessage.textContent = "Guess must contain 4 colours.";
        return;
    }
    try {
        const guessSnapshot = [...currentGuessSelection];

        const { game: updatedGame, score } = Mastermind.makeGuess(
            game,
            currentGuessSelection
        );
        ...
        attemptsRemaining.textContent = game.attemptsRemaining;

        const guessEntry = document.createElement("div");
        guessEntry.classList.add("guess-entry");
        guessEntry.textContent =
            `Guess: ${guessSnapshot.join(", ")} | ` +
            `Black Pegs: ${score.blackPegs} | ` +
            `White Pegs: ${score.whitePegs}`;
        guessHistory.prepend(guessEntry);

        currentGuessSelection = [];
        renderPegs(guessPreview, currentGuessSelection);

        if (game.winner === true) {
            gameMessage.textContent = "CODEBREAKER WINS";
        } else if (game.winner === false) {
            gameMessage.textContent =
                `CODEBREAKER LOSES - Secret code was: ` +
                `${game.secretCode.join(", ")}`;
        } else {
            gameMessage.textContent =
                `Black Pegs: ${score.blackPegs}, ` +
                `White Pegs: ${score.whitePegs}`;
